...
# st.chat_input is sticky at the bottom by default — exactly what we want.
# Voice is handled via a floating mic pill injected via CSS+JS into the page.
# When mic finishes, it fills a hidden st.text_input and triggers a rerun.

# Hidden voice text_input — Streamlit reads this; mic JS populates it
st.markdown("""
<style>
/* Push the hidden voice input off-screen but keep it in DOM so Streamlit can read it */
...
/* Extra bottom padding so chat history doesn't hide under the input bar */
...
# Floating mic button injected over the chat_input bar
components.html("""
<!DOCTYPE html>
<html>
<head>
...
#mic-fab {
  position: fixed;
  bottom: 18px;
  right: 72px;           /* sits just left of the send arrow */
  z-index: 9999;
  width: 36px;
  height: 36px;
  border-radius: 50%;
  ...
  box-shadow: 0 0 0 1.5px #7c3aed88;
  transition: background 0.15s, box-shadow 0.15s;
}
#mic-fab:hover { background: #2d1f5e; box-shadow: 0 0 0 2px #a78bfa; }
#mic-fab svg { width: 17px; height: 17px; color: #a78bfa; stroke: #a78bfa; }
...
<button id="mic-fab" title="Voice input (Hindi / Hinglish / English)" onclick="toggleMic()">
...
function showToast(msg, dur=2500) {
  ...
  toast._t = setTimeout(() => toast.classList.remove('toast-show'), dur);
}

function setVoiceInput(text) {
  // Target the hidden Streamlit text_input by its aria-label
...
function toggleMic() {
  ...
  if (!SR) { showToast('❌ Speech API needs Chrome/Edge'); return; }
  ...
  recog.onstart  = () => { listening = true;  fab.classList.add('listening');    showToast('🔴 Listening…', 9999); };
  recog.onend    = () => { listening = false; fab.classList.remove('listening'); };
  recog.onerror  = (e) => {
    listening = false; fab.classList.remove('listening');
...
# Main chat input (sticky bottom bar — the only visible input)
prompt = st.chat_input("Ask in English, Hindi, या Hinglish…")

# Pick up voice transcript if mic was used
