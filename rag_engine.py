...
class RAGEngine:
    ...
    def query(self, question: str) -> Dict[str, Any]:
        ...
        # 2. Enrich query with recent context for better retrieval
        #    (e.g., "tell me more" → add last assistant reply to query for embedding)
        history = self.memory.get_history_messages()
        if history and len(question.split()) < 8:
            # Short follow-up question — prepend last exchange for better embedding
        ...
        # 4. Build context block + system instruction
        summary_text = self.memory.get_summary_text()
        memory_note = (
            f"\n\nEARLIER CONVERSATION SUMMARY (use for context):\n{summary_text}"
