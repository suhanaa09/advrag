    ...
    if _raw.startswith("FILE:"):
        ...
        try:
            for fd in _j.loads(_raw[5:]):
                fname = fd["name"]
                if any(d["name"] == fname for d in st.session_state.uploaded_docs):
                    continue
                ext = fname.rsplit(".", 1)[-1].lower()
                fb  = _b.b64decode(fd["data"])
                if ext in ("pdf", "docx", "doc", "txt"):
                    ...
                    st.session_state.setdefault("pending_images", []).append({"name": fname, "b64": fd["data"], "mime": fd["mime"]})
                    st.toast(f"🖼 {fname} attached", icon="🖼️")
        except Exception as e:
            st.error(f"File error: {e}")
        st.rerun()
    elif _raw.startswith("VOICE:"):
...
# Step 5 — overlay iframe with + and mic, positioned over the chat bar
st.markdown("""
<style>
/* Pull the helper input fully out of flow */
[data-testid="stTextInput"]:has(#msg_bus) {
    display: none !important; height: 0 !important; margin: 0 !important;
}
/* Iframe overlay on the chat bar */
...
html, body { background: transparent; width: 100%; height: 48px; overflow: hidden; }
#fi { display: none; }
.bar {
...
.btn:hover { color: #e2e8f0; background: #ffffff12; }
#plus-btn { font-size: 22px; font-weight: 300; line-height: 1; margin-top: -1px; }
#mic-btn svg { width: 16px; height: 16px; stroke: currentColor; fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
.spacer { flex: 1; }
.rgap { width: 50px; }
.active { color: #f87171 !important; }
@keyframes glow { 0%,100%{filter:drop-shadow(0 0 3px #f87171aa)}50%{filter:none} }
.active svg { animation: glow 1s infinite; }
#toast {
    position: fixed; bottom: 56px; left: 50%; transform: translateX(-50%);
    background: #1e1b2e; border: 1px solid #7c3aed55; color: #c084fc;
    font-size: 11px; font-family: monospace; padding: 3px 12px;
    border-radius: 16px; opacity: 0; transition: opacity .2s;
...
<body>
<input type="file" id="fi" multiple accept=".pdf,.docx,.doc,.txt,.png,.jpg,.jpeg,.webp" onchange="onFiles(event)">
<div class="bar">
  <button id="plus-btn" class="btn" title="Attach" onclick="document.getElementById('fi').click()">+</button>
  <div class="spacer"></div>
  <button id="mic-btn" class="btn" title="Voice" onclick="toggleMic()">
    <svg viewBox="0 0 24 24"><rect x="9" y="2" width="6" height="12" rx="3"/><path d="M19 10a7 7 0 01-14 0"/><line x1="12" y1="19" x2="12" y2="22"/><line x1="8" y1="22" x2="16" y2="22"/></svg>
  </button>
  <div class="rgap"></div>
</div>
<div id="toast"></div>
...
async function onFiles(e) {
    ...
    sendToBus('FILE:' + JSON.stringify(out));
