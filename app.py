...
# ── FIX: Voice input widget (bidirectional via st.text_input hidden trick) ─────
# We use a real Streamlit text_input that gets populated by the mic widget below.
# The widget is hidden visually but Streamlit CAN read its value.
voice_key = "voice_input_field"

# Render mic widget (visual only — it writes to a Streamlit text_input via JS)
mic_html = """
<!DOCTYPE html>
<html>
<head>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: transparent; font-family: 'Inter', sans-serif; display:flex; align-items:center; justify-content:center; height:60px; }
  .icon-btn {
    background: #161b27; border: 1.5px solid #7c3aed; cursor: pointer;
    color: #a78bfa; padding: 10px; border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    transition: color 0.15s, background 0.15s;
  }
  .icon-btn:hover { background: #7c3aed33; }
  .icon-btn svg { width: 22px; height: 22px; }
  .mic-on { color: #f87171 !important; border-color: #f87171 !important; }
  @keyframes pulse {
    0%,100% { box-shadow: 0 0 0 0 rgba(248,113,113,.5); }
    50%      { box-shadow: 0 0 0 8px rgba(248,113,113,0); }
  }
  .mic-on { animation: pulse 1s ease infinite; }
  #status { font-size:11px; color:#64748b; margin-left:10px; font-family:monospace; }
</style>
</head>
<body>
...
function toggleMic() {
  ...
  recog.onresult = (e) => {
    ...
    status.textContent = '✓ ' + out.slice(0, 40) + (out.length > 40 ? '…' : '');
    // Send to Streamlit via postMessage — sets the voice_input_field value
...
# FIX: Use st.chat_input — the REAL, VISIBLE, WORKING input widget
# This is the primary fix: stop hiding it and use it properly.
prompt = st.chat_input("Ask in English, Hindi, या Hinglish…")

# ── FIX: Voice input section — placed ABOVE chat input area ──────────────────
st.markdown("---")
col_mic, col_voice_text = st.columns([1, 6])
with col_mic:
    # This renders the mic button widget
    components.html(mic_html, height=60, scrolling=False)
with col_voice_text:
    # FIX: Hidden text input that gets populated by the mic (via session state trick)
    # Users can also type here directly for voice-to-text confirmation
    voice_text = st.text_input(
        "🎤 Voice transcript (edit if needed, then press Enter)",
        key=voice_key,
        placeholder="Speak using the mic button, or type here…",
        label_visibility="visible",
    )
    if voice_text and voice_text != st.session_state.get("_last_voice", ""):
        st.session_state["_last_voice"] = voice_text
        prompt = voice_text  # treat voice input same as typed prompt


# ── Inline document upload in chat area ──────────────────────────────────────
with st.expander("📎 Quick-attach a document to chat", expanded=False):
...
# ── FIX: Process prompt (from chat input OR voice) ────────────────────────────
def run_query(user_prompt: str):
    """Core RAG query handler — called from both chat input and voice."""
    if not st.session_state.rag:
