...
# ── Welcome message (shown only when no messages yet) ─────────────────────────
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  #plus-btn:hover { color: #a78bfa; transform: rotate(90deg); background: #7c3aed22; }

  /* Mic / other icon buttons */
  .icon-btn {
    background: none; border: none; cursor: pointer;
...
<div id="bar">
  ...
  <!-- + Attach button -->
  <button id="plus-btn" title="Attach PDF / DOCX / TXT"
...
# ── Hidden file uploader — triggered by + button in bar ──────────────────────
# Completely hidden visually; the + button in the custom bar triggers it via JS
st.markdown("""
<style>
/* Hide the entire uploader widget — only the JS click trigger matters */
