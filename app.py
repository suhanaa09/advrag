...
/* Position the components iframe to sit flush over the chat input bar */
...
st.markdown("""
<style>
/* The iframe Streamlit renders gets a stCustomComponentV1 class */
/* We position it to float over the chat input bar */
iframe[title="st_custom_component"] {
    position: fixed !important;
    ...
    pointer-events: none !important;  /* let clicks fall through to chat input */
}
/* Re-enable pointer events only on the actual buttons inside */
iframe[title="st_custom_component"] { pointer-events: all !important; }
</style>
""", unsafe_allow_html=True)

# Floating + and mic — iframe overlaid on top of the chat bar
components.html("""
<!DOCTYPE html>
<html>
<head>
...
.btn:hover { color: #a78bfa; background: #7c3aed20; }

#plus-btn { font-size: 24px; font-weight: 300; line-height: 1; padding-bottom: 2px; }

#mic-btn svg { width: 17px; height: 17px; stroke: currentColor; fill: none;
...
@keyframes pulse {
  ...
  50%      { filter: drop-shadow(0 0 1px #f8717100); }
}
.active svg { animation: pulse 1s infinite; }
...
<!-- small gap so mic doesn't overlap send arrow -->
